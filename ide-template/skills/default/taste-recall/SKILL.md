---
name: taste-recall
description: Load relevant anti-pattern memory at session start. Read memory/patterns/ABOUT.md, then glob memory/patterns/*.md and pull in any whose `trigger:` frontmatter matches the current task. Use those rules as soft priors during the session. NOT user-facing — agent-facing memory primitive.
...
2. **Glob `memory/patterns/*.md`** (or use the `Glob` tool — your skill set has it).
