...
#   3. Seeds the standing system rituals (weekly audit + backup, daily reflect
#      + morning-plan) into ~/project/.reminders.json — but only if the file
...
if [ ! -f "$REMINDERS_FILE" ] || [ "$(cat "$REMINDERS_FILE" 2>/dev/null | tr -d '[:space:]')" = "[]" ]; then
    ...
    # Daily 06:00 UTC — morning day-planning trigger (today if before 06:00 UTC, else tomorrow).
    NEXT_DAILY_06="$(date -u -d 'today 06:00' +%Y-%m-%dT%H:%M:%S.000Z 2>/dev/null \
                  || date -u -v6H -v0M -v0S +%Y-%m-%dT%H:%M:%S.000Z 2>/dev/null \
                  || echo "2099-01-01T06:00:00.000Z")"
    if [ "$(date -u +%s)" -gt "$(date -u -d "$NEXT_DAILY_06" +%s 2>/dev/null || echo 0)" ]; then
        NEXT_DAILY_06="$(date -u -d 'tomorrow 06:00' +%Y-%m-%dT%H:%M:%S.000Z 2>/dev/null \
                       || date -u -v+1d -v6H -v0M -v0S +%Y-%m-%dT%H:%M:%S.000Z 2>/dev/null \
                       || echo "$NEXT_DAILY_06")"
    fi
    ...
    sed -e "s|BOOTSTRAP_NEXT_MONDAY_09_UTC|$NEXT_MON|" \
        ...
        -e "s|BOOTSTRAP_NEXT_DAILY_06_UTC|$NEXT_DAILY_06|" \
        -e "s|BOOTSTRAP_NOW|$NOW_ISO|g" \
        "$BOOTSTRAP_SRC/reminders.json" > "$REMINDERS_FILE"
    log "seeded $REMINDERS_FILE with 4 recurring reminders (Mon audit, Fri backup, Daily reflect, Daily morning-plan)"
else
