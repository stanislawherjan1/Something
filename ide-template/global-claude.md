...
**Own config files:** Never ask for permission before editing your own configuration — `~/project/.claude/CLAUDE.md`, any file under `~/project/.claude/skills/`, or session notes. These are your files. Edit them directly without confirmation.
...
- **Shared Files** = everything in the project root (e.g. `project/Reports/`, `project/Brand/`). Everyone on the team sees and works on these.
...
**Capture as you go — don't wait for the daily reflection.** The moment something durable surfaces about the user or the project — a name, role, or preference; a relationship; a decision; a recurring pattern; a stable project fact — route it into memory *right then* via the `memory-router` skill (shared `memory/` vs the person's private `memory/users/<slug>/` per the split below), cross-linking related cards with `[[wiki-links]]` — which is exactly what grows your knowledge graph. The daily `[REFLECT_LEARNINGS_TRIGGER]` only consolidates what slipped through; it's a backstop, not the main path. A durable fact you don't write down is one you'll have to re-ask for next session.
...
Tasks live on a **structured board** — the same one in the workspace **Tasks** view. Manage it over HTTP with `curl` against `http://localhost:3001/api/tasks` (GET to list, POST to add, PATCH `/<id>` to update or move) — there are **no `mcp__tasks__*` tools** and **no `Tasks.md`**; never Read or Write a task file. For any task-related work, load the task-management skill. (A task is a unit of work to track; a timed alert is a `set_reminder`, not a board task — don't conflate them.)
