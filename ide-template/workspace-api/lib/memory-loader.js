...
// seeding). Adding a card in one place and forgetting another is what leaked a
// private RESPONSIBILITIES card into group prompts; the registry removes that
// failure mode. Order there is LOCKED: reordering invalidates every existing
// prompt cache.
//
...
// topics/ and threads/. Only the tight, role-keyed summary cards are
// preloaded; those cards hold a one-line `→ concepts/<slug>.md` pointer,
// never the depth.
...
export function migrateDefaultMemory(adminSlug) {
  ...
  let moved = false;
  // ADOPT_CARDS, not "every user-tier card": USER_INDEX is user-tier but its
  // file IS the shared memory/INDEX.md, so the old LOAD_ORDER ∩ USER_TIER loop
  // moved the SHARED index into the admin's private dir — and once the admin had
  // one, unlinked memory/INDEX.md on EVERY boot (the detached reindex then
  // rebuilt it, so the damage was a self-healing flap nobody traced). The
  // registry's adopt flag also newly covers USER_RELATIONSHIPS / USER_REFLECTIONS,
  // which stayed flat — i.e. readable by every teammate — after team adoption.
  for (const { id, path } of ADOPT_CARDS) {
